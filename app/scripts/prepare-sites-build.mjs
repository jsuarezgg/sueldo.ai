...
for (const file of ["server/compare.js", "server/compare-output.js", "src/compensation.js", "src/share-link.js"]) {
...
const examples = [...guide.matchAll(/\[([^\]]+)\]\((https:\/\/sueldo\.ai\/compare[^)]+)\)/g)]
  .map(([, label, url]) => `<li><a href="${escape(url)}">${escape(label)}</a></li>`).join("");
...
<body><h1>Comparar ofertas desde un asistente de IA</h1><p>Sube tus ofertas en una conversación con acceso web y pide: Compara estos PDFs usando sueldo.ai. Lee primero https://sueldo.ai/ai y recupera una URL /compare con sus parámetros. Pregunta por lo que falte y no envíes datos personales.</p><p>Los parámetros de cálculo viajan en la URL y pueden quedar en registros. No envíes documentos ni datos identificadores.</p><p><a href="/">Calculadora</a> · <a href="/privacidad">Privacidad</a></p><h2>Ejemplos sintéticos</h2><ul>${examples}</ul><pre style="white-space:pre-wrap">${escape(guide)}</pre></body></html>`);
